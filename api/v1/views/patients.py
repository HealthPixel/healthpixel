...
@app_views.route(
    "/doctor/<doctor_id>/patient/<patient_id>", methods=["GET"], strict_slashes=False
)
def get_a_patient(doctor_id, patient_id):
    ...
    access_log = Access_Log(
        user_id=doctor_id, patient_id=patient_id, action_taken=action_taken
    )
...
def search_patients():
    ...
    matching_patients = [
        ...
        if query in patient.email.lower() or query == patient.id.lower()
    ]
...
@app_views.route(
    "/doctor/<doctor_id>/patient/<patient_id>", methods=["PUT"], strict_slashes=False
)
...
@app_views.route(
    "/doctor/patients/<patient_id>/update_patient_records",
    methods=["GET", "POST"],
...
def update_patient_records(patient_id):
    ...
    if request.method == "GET":
        ...
        medical_record = (
            storage.query(Medical_Record).filter_by(patient_id=patient_id).first()
        )
        allergies = storage.query(Allergies).filter_by(patient_id=patient_id).first()
        appointment = (
            storage.query(Appointment).filter_by(patient_id=patient_id).first()
        )
        ...
        try:
            # Vitals Update
            if vitals:
                vitals.blood_pressure = data.get(
                    "blood_pressure", vitals.blood_pressure
                )
                vitals.heart_rate = data.get("heart_rate", vitals.heart_rate)
                vitals.body_temperature = data.get(
                    "body_temperature", vitals.body_temperature
                )
                vitals.respiratory_rate = data.get(
                    "respiratory_rate", vitals.respiratory_rate
                )
                vitals.oxygen_saturation = data.get(
            ...
            if medical_record:
                medical_record.diagnosis = data.get(
                    "diagnosis", medical_record.diagnosis
                )
                medical_record.treatment = data.get(
                    "treatment", medical_record.treatment
                )
                medical_record.status = data.get("status", medical_record.status)
                medical_record.notes = data.get(
                    "medical_record_notes", medical_record.notes
                )
                ...
                if "submit_medical_record" in data:
                    ...
                    new_medical_record = Medical_Record(
                        diagnosis=diagnosis,
                        treatment=treatment,
                        status=status,
                        notes=notes,
            ...
            if lab_result:
                ...
                if "submit_lab_result" in data:
                    ...
                    new_lab_result = Lab_Results(
                        test_name=test_name, result=result, values=values, notes=notes
                    )
            ...
            # Medication Update
            if medication:
                medication.medication_name = data.get(
                ...
                medication.prescribing_doctor = data.get(
            ...
            return redirect(
                url_for("app_views.update_patient_records", patient_id=patient_id)
            )
...
@app_views.route(
    "/doctor/patients/<patient_id>/view_patient_records",
    methods=["GET"],
    strict_slashes=False,
)
@login_required
def view_patient_records(patient_id):
    ...
    access_log = Access_Log(
        user_id=current_user.id, patient_id=patient_id, action_taken=action_taken
    )
    storage.new(access_log)
    storage.save()
...
@app_views.route(
    "/doctor/<doctor_id>/patient/<patient_id>", methods=["DELETE"], strict_slashes=False
)
